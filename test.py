# Rewriting joint orient in python

import maya.cmds as cmds

def OrientJoints(joints):
    aimAxis = "x"
    upDir = cmds.upAxis(q=True, ax=True)
...
def CrossAngle(pos1, pos2, pos3):
    ...
    magnitude = math.sqrt(sum([a**2 for a in cross]))
    return [a/magnitude for a in cross]


sel = cmds.ls(sl=True)
cross = CrossAngle(sel[0], sel[1], sel[2])
cmds.aimConstraint(
    aim=(1,0,0),
    upVector=(0,1,0),
    worldUpVector=cross,
